...
{
    public class CarreraNegocios(ICarreraRepositorios carreraRepositorio)
    {
        private readonly ICarreraRepositorios _carreraRepositorio = carreraRepositorio;
        ...
        public async Task<ResultadoAcciones> InsertarCarrera(E_Carrera carrera)
        {
            ...
            try
            {
                var dataRes = await _carreraRepositorio.InsertarCarrera(carrera);
                return dataRes;
            }
            catch (Exception ex)
            {
                return new ResultadoAcciones 
                {
                    Mensajes = { " Ocurrio un error inesperado al insertar la carrera: ", ex.Message + "." },
                    Resultado = false
                };
            }
        }
        ...
        {
            if (_carreraRepositorio == null || carrera.IdCarrera <= 0)
            {
                return new ResultadoAcciones
                {
            ...
            // Validar la entidad recibida, no el repositorio
        ...
        private async Task ValidarUnicidadClaveCarrera(ResultadoAcciones resultado, string? claveCarrera, int? idExcluido = null)
        {
            ...
            var clave = claveCarrera.Trim();
            bool existe = await _carreraRepositorio.ExisteClaveCarrera(clave);
            if (!existe) return;
